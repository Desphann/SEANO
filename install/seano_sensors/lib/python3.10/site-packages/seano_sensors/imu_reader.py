...
class IMUReader(Node):
    ...
    def imu_callback(self, msg):
        ...
        if not self.has_ever_received_data:
            ...
            self.get_logger().info(f"IMU connected | first data received at {timestamp}")
        ...
        if self.timeout_reported:
            timestamp = self.convert_time(msg.header.stamp)
            self.get_logger().info(f"IMU reconnected | data received again at {timestamp}")
            self.is_connected = True
            self.timeout_reported = False
    ...
    def check_status(self):
        ...
        if self.last_msg_time is None:
            if (now - self.last_waiting_log_time) >= self.waiting_reminder_sec:
                self.get_logger().warning("IMU still waiting for data...")
                self.last_waiting_log_time = now
            return

        if (now - self.last_msg_time) > self.timeout_sec:
            if not self.timeout_reported:
                self.get_logger().error("IMU timeout | no data received in the last 5 seconds")
